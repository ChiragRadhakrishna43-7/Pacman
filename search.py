...
def breadthFirstSearch(problem):
    ...
    frontier_nodes.push(start_node)
    explored = []
    while not frontier_nodes.isEmpty():
        ...
        for action in problem.getActions(current_state):
            next_state = problem.getResult(current_state,action)
            if next_state not in explored:
                next_node = Node(next_state, current, action, 0)
...
def aStarSearch(problem, heuristic):
    ...
    frontier_nodes.push(start_node,0)
    explored = []
    while not frontier_nodes.isEmpty():
        current = frontier_nodes.pop()
        current_state = current.state
        ...
        for action in problem.getActions(current_state):
            next_state = problem.getResult(current_state, action)
            if next_state not in explored:
                g = current.path_cost + problem.getCost(current_state, action)
                h = heuristic(next_state, problem)
                f = g + h
                next_node = Node(next_state, current, action, g)
                frontier_nodes.push(next_node, f)
    return []
    util.raiseNotDefined()
